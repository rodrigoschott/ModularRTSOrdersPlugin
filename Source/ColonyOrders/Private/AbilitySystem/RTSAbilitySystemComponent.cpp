...
void URTSAbilitySystemComponent::AddItemAbility(TSubclassOf<UGameplayEffect> GameplayEffectClass)
{
    ...
    // Try to get abilities from the new component system first
    TArray<FGameplayAbilitySpecDef> GrantedAbilities;
    
    // In UE 5.5, GameplayEffects use a component system for abilities
    // Find AbilitiesGameplayEffectComponent using reflection on the effect's components
    bool bFoundAbilitiesFromComponents = false;
    
    // Use reflection to find components since we don't have direct access to the component class
    UClass* EffectClass = GameplayEffect->GetClass();
    for (TFieldIterator<FProperty> PropIt(EffectClass); PropIt; ++PropIt)
    {
        FProperty* Property = *PropIt;
        if (Property && Property->GetName().Contains(TEXT("Component")))
        {
            // Try to access this property as a potential component
            if (FObjectProperty* ObjectProp = CastField<FObjectProperty>(Property))
            {
                if (UObject* Component = ObjectProp->GetObjectPropertyValue_InContainer(GameplayEffect))
                {
                    // Check if this component has GrantedAbilities
                    if (FProperty* AbilitiesProp = Component->GetClass()->FindPropertyByName(TEXT("GrantedAbilities")))
                    {
                        if (FArrayProperty* ArrayProp = CastField<FArrayProperty>(AbilitiesProp))
                        {
                            FScriptArrayHelper ArrayHelper(ArrayProp, ArrayProp->ContainerPtrToValuePtr<void>(Component));
                            for (int32 i = 0; i < ArrayHelper.Num(); ++i)
                            {
                                void* AbilityDefPtr = ArrayHelper.GetRawPtr(i);
                                if (FStructProperty* StructProp = CastField<FStructProperty>(ArrayProp->Inner))
                                {
                                    if (StructProp->Struct->GetFName() == "GameplayAbilitySpecDef")
                                    {
                                        FGameplayAbilitySpecDef* AbilityDef = static_cast<FGameplayAbilitySpecDef*>(AbilityDefPtr);
                                        GrantedAbilities.Add(*AbilityDef);
                                        bFoundAbilitiesFromComponents = true;
                                    }
                                }
                            }
                        }
    ...
    // If no abilities found from components, fall back to deprecated GrantedAbilities
    if (!bFoundAbilitiesFromComponents)
    {
        // Fallback to deprecated GrantedAbilities for older versions or compatibility
PRAGMA_DISABLE_DEPRECATION_WARNINGS
        GrantedAbilities = GameplayEffect->GrantedAbilities;
PRAGMA_ENABLE_DEPRECATION_WARNINGS
    ...
    for (const FGameplayAbilitySpecDef& ItemAbilityDef : GrantedAbilities)
    {
...
void URTSAbilitySystemComponent::RemoveItemAbility(TSubclassOf<UGameplayEffect> GameplayEffectClass)
{
    ...
    // UE 5.5: Use GetAssetTags() instead of deprecated InheritableGameplayEffectTags
    FGameplayTagContainer TagContainer;
    
    // Try the new method first
    if (GameplayEffect->GetClass()->FindFunctionByName(TEXT("GetAssetTags")))
    {
        TagContainer = GameplayEffect->GetAssetTags();
    }
    else
    {
        // Fallback to deprecated method if GetAssetTags doesn't exist
PRAGMA_DISABLE_DEPRECATION_WARNINGS
        TagContainer = GameplayEffect->InheritableGameplayEffectTags.CombinedTags;
PRAGMA_ENABLE_DEPRECATION_WARNINGS
    }
...
void URTSAbilitySystemComponent::InitializeAttributes(int AttributeLevel, bool bInitialInit)
{
    ...
    */
    ...
    for (const FAttributeDefaults& DefaultEntry : DefaultStartingData) // Iterate your blueprint-configured defaults
    {
        if (const UAttributeSet* AS = GetAttributeSet(DefaultEntry.Attributes)) // Get the spawned instance
        {
            if (const URTSAttributeSet* RTSAS = Cast<URTSAttributeSet>(AS))
            {
                const_cast<URTSAttributeSet*>(RTSAS)->PostInitializeProperties(bInitialInit);
            }
        }
    }
