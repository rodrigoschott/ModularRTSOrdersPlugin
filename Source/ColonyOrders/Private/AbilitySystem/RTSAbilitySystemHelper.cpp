...
{
    return GameplayEffectSpec.GetStackCount();
}
...
URTSAbilitySystemHelper::GetGrantedAbilitiesFromGameplayEffect(TSubclassOf<UGameplayEffect> GameplayEffectClass)
{
    ...
    // UE 5.5 Update: In UE 5.5, GameplayEffects use components for granted abilities
    // but accessing UGameplayEffectComponent directly causes incomplete type errors.
    // Fall back to accessing the deprecated GrantedAbilities property for now.
    
#if WITH_EDITORONLY_DATA
PRAGMA_DISABLE_DEPRECATION_WARNINGS
    // Note: GrantedAbilities is deprecated in UE 5.5 but still accessible for compatibility
